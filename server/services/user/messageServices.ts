  ...
  try {
    ...
    if (!isAi && content && content.trim()) {
      ...
      try {
        ...
        console.error(
          "Failed to generate embedding for Telegram user message:",
          embeddingError
        );
        // Continue without embedding if generation fails
      }
      ...
      // Create AI response message with additional AI data
      const aiMessageData: InsertMessage = {
        chatId,
        userId,
        content: aiResult.data?.response || "",
      ...
      try {
        ...
        console.error(
          "Failed to generate embedding for Telegram AI message:",
          embeddingError
        );
        // Continue without embedding if generation fails
      }
      ...
      // Return AI response with whatsappSummary for Telegram
      return {
        data: aiResponse,
        message: "Telegram AI response generated successfully",
        error: null,
        aiResult: aiResult, // Include the full AI result for access to whatsappSummary
      };
    } else {
      // For AI messages or messages without content, generate embedding if content exists
      if (content?.trim()) {
        try {
          const embedding = await embeddingService.generateEmbedding(content);
          messageData.embedding = embedding;
          ...
          console.error(
            "Failed to generate embedding for Telegram message:",
            embeddingError
          );
          // Continue without embedding if generation fails
        }
      ...
      return {
        data: newMessage,
        message: "Telegram message created successfully",
        error: null,
      };
    }
  } catch (error) {
    return { data: null, message: "Failed to create Telegram message", error };
  }
};
